...
<img alt="Harness Score L4" src="https://paladini.github.io/harness-score/maturity/badge-l4.svg" height="20">
**A reproducible study of AI-harness maturity across notable open-source
repositories** â€” scored deterministically with
...
> excluded for a documented platform limitation, not silently dropped â€”
...
> â€” do the automated levels agree with a blind human read?) is Phase 2 and
> needs a human rater â€” see [Roadmap](#roadmap).
...
prompt/eval engineering projects, and artifact-governance collections â€”
...
owns it. A frontier lab's inference library can legitimately score low â€”
...
`harness-score@1.0.0`. A representative spread â€” full table (all 20, every
...
|---|---|---|---|
| [harness-score](https://github.com/paladini/harness-score) | control Â· ceiling | **L4** | 108/108 (100%) |
...
| [execa](https://github.com/sindresorhus/execa) | control Â· quality w/o AI artifacts | **L0** | 36/108 (33%) |
| [anthropic-skills](https://github.com/anthropics/skills) | harness-engineering exemplar | **L0** | 17/108 (16%) |
| [octocat/Hello-World](https://github.com/octocat/Hello-World) | control Â· floor | **L0** | 14/108 (13%) |
...
fakeflix â€” previously validated in harness-score's own v0.1.2 field test
as "genuinely excellent" â€” earns 67% of all points but is capped at **L1**
because Context & Guides sits at 45%: a substantive root `AGENTS.md` (79
non-empty lines, 17 headings â€” passes outright) but **zero scoped rule
files**, so `CTX-03` through `CTX-06` all fail, even as Skills & Commands
sits at 82% and Sensors & Feedback at 100%. The same shape â€” real root
context file, no scoped rules â€” turned out to be common in the wider
...
`anthropic/skills` â€” Anthropic's official showcase of Claude Skills â€”
scores **L0 Â· 16%**, indistinguishable in kind from `octocat/Hello-World`.
...
using them to develop itself â€” and `SKL-01` correctly answers the question
...
at all" â€” a `.claude-plugin/` manifest at root (which this repo has) is a
...
[check-change process](https://github.com/paladini/harness-score/blob/main/CONTRIBUTING.md#adding-or-changing-a-check) â€”
...
2. [`corpus/run.mjs`](corpus/run.mjs) clones each pinned commit into a
   ...
   --json` against it â€” no code from the scanned repository is ever
...
3. [`corpus/build-results.mjs`](corpus/build-results.mjs) turns the raw
   ...
   `results/dimension-heatmap.md` â€” deterministically, no hand-edited
   tables.
4. A human blind rating (recorded *before* seeing the tool's score â€”
   protocol in
   ...
   yet** â€” this is the part of the study that needs a human, not an agent.
...
Everything is pinned and versioned â€” the raw JSON report for every scanned
...
The corpus is curated and pinned on purpose â€” but the scanner behind it
...
and tells you where it would land among the current corpus â€” without
...
Packaged as a Claude Code skill â€”
[`score-any-repo`](.claude/skills/score-any-repo/SKILL.md) â€” for "how does
...
scans itself at **L4 Â· Self-correcting â€” 96/108 (89%)**: a scoped `.cursor/rules/`
...
```bash
npm install
npm test           # vitest â€” pure functions, parseArgs, manifest.json shape
npm run lint        # biome
```
...
- [x] **Phase 1.** Corpus frozen and scanned: 20/21 pinned repositories
      ...
      dropped).
- [ ] **Phase 2.** Blind human ratings + per-repo critique â€” needs a rater
      without implementation knowledge of the scanner. Not started.
- [x] **Phase 3 (Q2 only).** Model-calibration synthesis â€”
...
[MIT](LICENSE) Â© 2026 Fernando Paladini
